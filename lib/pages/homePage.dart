...
class tripState extends State<trips> with SingleTickerProviderStateMixin {
  ...
  List<Trip> tripslist = [];        // starts empty, filled from Firebase             //to open the drawer from the header

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 3, vsync: this);
    _loadTrips(); //load from Firebase
  }
  ...
  void _loadTrips() {
    DatabaseService.tripsRef.onValue.listen((event) {
      if (!mounted) return;
      final data = event.snapshot.value;
      if (data == null) { setState(() => tripslist = []); return; }
  ...
  Widget headerBar() {
    final cities = 1;                                          //to displya on top
    final places = 3;
  ...
  void _deleteTrip(Trip trip) {                              //alert dialog before deleting
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(
            onPressed: () async{//remove the trip
              Navigator.pop(context);
              await DatabaseService.tripsRef.child(trip.firebaseKey).remove(); //firebase delete
                  ...
                  actions: [
                    TextButton(
                        onPressed: () async {
                          ...
                          await DatabaseService.tripsRef.push().set({ // saves to Firebase
  ...
  Widget tripContainer(Trip trip){
    return GestureDetector(
      onTap: () => Navigator.pushReplacementNamed(context, '/cities', arguments: trip),//tap the card to go to cities
      child: Container(
  ...
  Widget tripGridCard(Trip trip) {                           //card design for the history grid
    return GestureDetector(
        onTap: () => Navigator.pushReplacementNamed(context, '/cities', arguments: trip),
        child: Container(
            decoration: BoxDecoration(
                color: Colors.white,
                borderRadius: BorderRadius.circular(20),
    ...
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: const Icon(Icons.search, color: Colorpalette.steelBlue),
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _searchQuery.isEmpty ? tripslist       //seprate the trips by their status
        : tripslist.where((t) =>
    t.name.toLowerCase().contains(_searchQuery) ||
        t.status.toLowerCase().contains(_searchQuery)).toList();
    final active   = filtered.where((t) => t.status == 'active').toList();
    final planning = filtered.where((t) => t.status == 'planning').toList();
    final history  = filtered.where((t) => t.status == 'history').toList();
    return Scaffold(
          ...
          child: Row( mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
                  ...
                  onTap: () {
                    if (tripslist.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Add a trip first!'), behavior: SnackBarBehavior.floating));
                      return;
                    }
                    showDialog(context: context, builder: (_) => AlertDialog(
                      title: const Text('Select a Trip', style: TextStyle(fontFamily: 'Nunito', fontWeight: FontWeight.bold)),
                      content: SizedBox(width: double.maxFinite,
                          child: ListView.builder(
                            shrinkWrap: true,
                            itemCount: tripslist.length,
                            itemBuilder: (_, i) => ListTile(
                              leading: Text(tripslist[i].flag, style: const TextStyle(fontSize: 22)),
                              title: Text(tripslist[i].name, style: const TextStyle(fontFamily: 'Nunito', fontWeight: FontWeight.bold)),
                              subtitle: Text(tripslist[i].dates, style: const TextStyle(fontFamily: 'DMSans', fontSize: 12)),
                              onTap: () {
                                Navigator.pop(context);
                                Navigator.pushReplacementNamed(context, '/cities', arguments: tripslist[i]);
                              },
                            ),
                          )),
                    ));
              ...
              GestureDetector(                                //go to days
                  onTap: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Open a city from the Cities page to view days.'),
                            behavior: SnackBarBehavior.floating));
                  },
                  child: const Column( mainAxisSize: MainAxisSize.min, children: [
